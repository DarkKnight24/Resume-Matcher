					...
					<div className="p-6 max-h-[70vh] overflow-y-auto">
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
							<div className="md:col-span-1 bg-gray-800 p-4 rounded-lg">
								<h4 className="text-lg font-semibold text-blue-400 mb-2">{t('overallScore')}</h4>
							...
							<div className="md:col-span-2 bg-gray-800 p-4 rounded-lg">
								<h4 className="text-lg font-semibold text-blue-400 mb-2">{t('summary')}</h4>
								<p className="text-gray-300 text-sm mb-1">
									<strong>{t('details')}:</strong> {details}
								</p>
								<p className="text-gray-300 text-sm">
									<strong>{t('commentary')}:</strong> {commentary}
								</p>
							</div>
						</div>

						<div>
							<h4 className="text-xl font-semibold text-blue-400 mb-3">{t('improvementSuggestions')}</h4>
								...
								<ul className="space-y-3">
									{improvements.map((item, idx) => (
										<li key={idx} className="bg-gray-800 p-4 rounded-md shadow">
											...
											{item.lineNumber && (
												<p className="text-xs text-gray-500 mt-1">{t('reference')} {item.lineNumber}</p>
											)}
										</li>
									))}
								</ul>
							) : (
								<p className="text-gray-400 text-sm">{t('noImprovementSuggestions')}</p>
							)}
						</div>
					</div>
